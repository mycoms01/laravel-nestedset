...
trait NodeTrait
{
    ...
    public function createPath($parrent)
    {
        $values = $parrent ? explode('/', $parrent->path) : [];
        $values[] = $this->getKey();
        $this->setPath(implode('/', $values))->save();
        return $this->getPath();
...
//        $this->original = array_merge($this->original, $attributes);
    ...
    {
        $this->attributes[$this->getPathName()] = $value;
